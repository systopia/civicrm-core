...
class CRM_Mailing_MailStore_Imap extends CRM_Mailing_MailStore {
  ...
  function __construct($host, $username, $password, $ssl = TRUE, $folder = 'INBOX') {
    ...
    $this->_ignored   = implode($this->_transport->getHierarchyDelimiter(), array('CiviMail', 'ignored'));
    $this->_processed = implode($this->_transport->getHierarchyDelimiter(), array('CiviMail', 'processed'));

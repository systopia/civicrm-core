...
class CRM_Utils_String {
  ...
  public static function parseOneOffStringThroughSmarty($templateString, $templateVars = []) {
    ...
    try {
      self::parseWithSmarty($smarty, $templateString, $templateVars);
    }
    catch (Exception $e) {
      ...
      // TODO: Remove insecure retry once all {crmAPI} tags have been removed
      //       from templates.
      if (defined('CIVICRM_SMARTY_INSECURE_RETRY') && CIVICRM_SMARTY_INSECURE_RETRY) {
        $exeptionMessage = 'Message was not parsed due to invalid smarty syntax : '
          . $e->getMessage()
          . (
            (CIVICRM_UF === 'UnitTest' || CRM_Utils_Constant::value('SMARTY_DEBUG_STRINGS'))
              ? $templateString
              : ''
          );
        \Civi::log('smarty')->warning(
          '[PATCHED] Insecure retry of Smarty rendering due to exception thrown: {exception}',
          ['exception' => $exeptionMessage]
        );
        ...
        try {
          ...
          throw new \CRM_Core_Exception(
            'Message was not parsed due to invalid smarty syntax : '
            . $e->getMessage()
            . (
              (CIVICRM_UF === 'UnitTest' || CRM_Utils_Constant::value('SMARTY_DEBUG_STRINGS'))
                ? $templateString
        ...
        // TODO: The exception needs to be thrown unconditionally when removing
        //       the insecure retry rendering.
  ...
  /**
   * Parses a given string with optional template variables using a given smarty
   * instance.
   *
   * TODO: This used to be inside parseOneOffStringThroughSmarty() but needed to
   *       be called twice with the insecure retry patch.
   */
  private static function parseWithSmarty(CRM_Core_Smarty &$smarty, string &$templateString, array &$templateVars = []): void {

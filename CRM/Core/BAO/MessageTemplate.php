  ...
  public static function sendTemplate($params) {
    ...
    if ($params['toEmail']) {
      ...
      if (isset($params['isEmailPdf']) && $params['isEmailPdf'] == 1) {
        $pdfHtml = CRM_Contribute_BAO_ContributionPage::addInvoicePdfToEmail($params['contributionId'], $params['contactId'], $params['emailReceipt']);
        if (empty($params['attachments'])) {
          $params['attachments'] = array();
        }
        if ($params['emailReceipt']) {
          $params['attachments'][] = CRM_Utils_Mail::appendPDF('Payment_Receipt.pdf', $pdfHtml, $format);
        }
        else {

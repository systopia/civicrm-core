 ...
 *
 */
class CRM_Contact_Form_Search_Custom_ICACustom extends CRM_Contact_Form_Search_Custom_Base implements CRM_Contact_Form_Search_Interface {
  function __construct(&$formValues) {
    ...
    if ($types == 'organisations') {
      $this->_columns = array(
        ...
        ts('Name') => 'sort_name',
        ts('Individual Name') => 'individual_name',
  ...
  function buildForm(&$form) {
    ...
    CRM_Core_BAO_CustomField::addQuickFormElement($form, 'country', '8', TRUE, FALSE, TRUE, 'Country');
    ...
    */
     ...
     */
    $form->assign('elements', array('sort_name', 'superregion', 'region', 'country', 'member', 'status', 'tag', 'language', 'children', 'issues', 'types'));
    ...
    if ($types == 'organisations') {
      ...
      if (array_sum($issues_no_op) > 0) {
        $selectClause = "
          ...
          rel_indiv_contact.sort_name as individual_name  ,
  ...
  function from() {
    ...
    $country = CRM_Utils_Array::value('country', $this->_formValues);
  ...
  function where($includeContactIDs = TRUE) {
    ...
    if ($country) {
      for ($i = 0; $i <= $children ; $i++) {
        $country_clauses[] = "locationp$i.country_8 LIKE %$placeholder";
        $params[$placeholder] = array($country, 'String');
        $placeholder++;
        if ($types != 'organisations') {
          $country_clauses[] = "rel_org_locationp$i.country_8 LIKE %$placeholder";
          $params[$placeholder] = array($country, 'String');
          $placeholder++;
        }
      }
      $clauses[] = ' ( ' . implode(' OR ', $country_clauses) . ' ) ';
    }
    //$type = gettype($member);
